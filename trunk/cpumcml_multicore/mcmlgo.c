...
float ran3(int *idum, int pid) {
  long mj, mk;
  int i, ii, k;

  //pthread_mutex_lock (&ran3lock);

  if (*idum < 0 || ranparm[pid].iff == 0) {
    ranparm[pid].iff=1;
    mj=MSEED-(*idum < 0 ? -*idum : *idum);
    mj %= MBIG;
    ranparm[pid].ma[55]=mj;
    mk=1;
    for (i=1; i<=54; i++) {
      ii=(21*i) % 55;
      ranparm[pid].ma[ii]=mk;
      mk=mj-mk;
      if (mk < MZ)
        mk += MBIG;
      mj=ranparm[pid].ma[ii];
    }
    for (k=1; k<=4; k++)
      for (i=1; i<=55; i++) {
        ranparm[pid].ma[i] -= ranparm[pid].ma[1+(i+30) % 55];
        if (ranparm[pid].ma[i] < MZ)
          ranparm[pid].ma[i] += MBIG;
      }
    ranparm[pid].inext=0;
    ranparm[pid].inextp=31;
    *idum=1;
  }
  if (++ranparm[pid].inext == 56)
    ranparm[pid].inext=1;
  if (++ranparm[pid].inextp == 56)
    ranparm[pid].inextp=1;
  mj=ranparm[pid].ma[ranparm[pid].inext]-ranparm[pid].ma[ranparm[pid].inextp];
  if (mj < MZ)
    mj += MBIG;
  ranparm[pid].ma[ranparm[pid].inext]=mj;
...
double RandomNum(int pid) {
  if (first_time[pid]) {
#if STANDARDTEST /* Use fixed seed to test the program. */
    //idum[pid] = - 1;
    idum[pid] = -1237-pid; //need to add pid to ensure the seeds are not repeating...
#else
    //idum[pid] = -(int)time(NULL)%(1<<15)+pid; //need to add pid to ensure the seeds are not repeating...
  ...
  return ( (double)ran3(&idum[pid], pid) );
}
...
double Rspecular(LayerStruct * Layerspecs_Ptr) {
  ...
  temp =(Layerspecs_Ptr[0].n - Layerspecs_Ptr[1].n) /(Layerspecs_Ptr[0].n
      + Layerspecs_Ptr[1].n);
  r1 = temp*temp;

  if ((Layerspecs_Ptr[1].mua == 0.0) && (Layerspecs_Ptr[1].mus == 0.0)) { /* glass layer. */
    temp = (Layerspecs_Ptr[1].n - Layerspecs_Ptr[2].n) /(Layerspecs_Ptr[1].n
        + Layerspecs_Ptr[2].n);
    r2 = temp*temp;
 ...
 ****/
void LaunchPhoton(double Rspecular, LayerStruct * Layerspecs_Ptr,
 ...
 ****/
double SpinTheta(double g, int pid) {
  double cost;

  if (g == 0.0)
    cost = 2*RandomNum(pid) -1;
  else {
    double temp = (1-g*g)/(1-g+2*g*RandomNum(pid));
 ...
 ****/
void Spin(double g, PhotonStruct * Photon_Ptr, int pid) {
  ...
  cost = SpinTheta(g, pid);
  sint = sqrt(1.0 - cost*cost);
  /* sqrt() is faster than sin(). */

  psi = 2.0*PI*RandomNum(pid); /* spin psi 0-2pi. */
  cosp = cos(psi);
  if (psi<PI)
 ...
 ****/
void StepSizeInTissue(PhotonStruct * Photon_Ptr, InputStruct * In_Ptr, int pid) {
  short layer = Photon_Ptr->layer;
 ...
 ****/
void Roulette(PhotonStruct * Photon_Ptr, int pid) {
  if (Photon_Ptr->w == 0.0)
    Photon_Ptr->dead = 1;
  else if (RandomNum(pid) < CHANCE) /* survived the roulette.*/
    Photon_Ptr->w /= CHANCE;
  else
    Photon_Ptr->dead = 1;
}
...
InputStruct * In_Ptr, PhotonStruct * Photon_Ptr, OutStruct * Out_Ptr) {
  double x = Photon_Ptr->x;
  double y = Photon_Ptr->y;
 ...
 *	Update the photon weight as well.
 ****/
void RecordT(double Refl, InputStruct * In_Ptr, PhotonStruct * Photon_Ptr,
    OutStruct * Out_Ptr) {
  double x = Photon_Ptr->x;
  double y = Photon_Ptr->y;
  short ir, ia; /* index to r & angle. */
 ...
 ****/
void CrossUpOrNot(InputStruct * In_Ptr, PhotonStruct * Photon_Ptr,
    OutStruct * Out_Ptr, int pid) {
  ...
  else if(RandomNum(pid) > r) {/* transmitted to layer-1. */
    Photon_Ptr->layer--;
    Photon_Ptr->ux *= ni/nt;
    Photon_Ptr->uy *= ni/nt;
    Photon_Ptr->uz = -uz1;
  ...
  Photon_Ptr->uz = -uz;
#else
  if (RandomNum(pid) > r) { /* transmitted to layer-1. */
    if (layer==1) {
      Photon_Ptr->uz = -uz1;
 ...
 *	Update the photon parmameters.
 ****/
void CrossDnOrNot(InputStruct * In_Ptr, PhotonStruct * Photon_Ptr,
    OutStruct * Out_Ptr, int pid) {
  double uz = Photon_Ptr->uz; /* z directional cosine. */
...
void CrossOrNot(InputStruct * In_Ptr, PhotonStruct * Photon_Ptr,
  ...
  if (Photon_Ptr->uz < 0.0)
    CrossUpOrNot(In_Ptr, Photon_Ptr, Out_Ptr, pid);
  else
    CrossDnOrNot(In_Ptr, Photon_Ptr, Out_Ptr, pid);
}
 ...
 ****/
void HopInGlass(InputStruct * In_Ptr, PhotonStruct * Photon_Ptr,
    OutStruct * Out_Ptr, int pid) {
  ...
  if (Photon_Ptr->uz == 0.0) {
    ...
    Hop(Photon_Ptr);
    CrossOrNot(In_Ptr, Photon_Ptr, Out_Ptr, pid);
  }
}
 ...
 ****/
void HopDropSpinInTissue(InputStruct * In_Ptr, PhotonStruct * Photon_Ptr,
    OutStruct * Out_Ptr, int pid) {
  StepSizeInTissue(Photon_Ptr, In_Ptr, pid);
  ...
  if (HitBoundary(Photon_Ptr, In_Ptr)) {
    ...
    Spin(In_Ptr->layerspecs[Photon_Ptr->layer].g, Photon_Ptr, pid);
  }
}
...
void HopDropSpin(InputStruct * In_Ptr, PhotonStruct * Photon_Ptr,
  ...
  if ((In_Ptr->layerspecs[layer].mua == 0.0) && (In_Ptr->layerspecs[layer].mus
      == 0.0))
    /* glass layer. */
    HopInGlass(In_Ptr, Photon_Ptr, Out_Ptr, pid);
  else
    HopDropSpinInTissue(In_Ptr, Photon_Ptr, Out_Ptr, pid);

  if (Photon_Ptr->w < In_Ptr->Wth && !Photon_Ptr->dead)
    Roulette(Photon_Ptr, pid);
}
